...
mysql_query("CREATE TABLE IF NOT EXISTS `$tableName` (
  `id` int(11) NOT NULL AUTO_INCREMENT, `Date` Date NOT NULL, `Receipt_No` varchar(60) NOT NULL, `agent_email` varchar(60) NOT NULL, `Sender` varchar(60) NOT NULL,
...
if(($bs_mtls=='1')){
mysql_query ( "INSERT $tableName(id,Date,Receipt_No,agent_email,Sender,Receiver,Transfer_Mode,Amount_sending,Local_Pay_N,Comm,Agent_Com,Total_Paying,status)

SELECT id,date,receiptno,agent_email,sender_name,r_name,r_transfer,amt_send,amt_local,com_d,com_a,total,status FROM agent_cust_transaction WHERE `amt_send` <= $bs_mtl AND STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts3' AND '$ts4'   && agent_email = '$aemail '");
}

if(($bs_mtls=='0')){
mysql_query ( "INSERT $tableName(id,Date,Receipt_No,agent_email,Sender,Receiver,Transfer_Mode,Amount_sending,Local_Pay_N,Comm,Agent_Com,Total_Paying,status)

SELECT id,date,receiptno,agent_email,sender_name,r_name,r_transfer,amt_send,amt_local,com_d,com_a,total,status FROM agent_cust_transaction WHERE STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts3' AND '$ts4'  && agent_email = '$aemail'");
}
