		...
		$queryr2 = $db->query("SELECT * FROM agent_cust_receiver WHERE agent_email='$email' && (b_fname LIKE '$sv%' || b_lname LIKE '$sv%' || b_phone LIKE '$sv%') LIMIT 5");
				if($querycs) {
					while ($resultst = $querycs ->fetch_object()) {
						...
						$s_mphone=$resultst->mnumber;
						$c_name=$fname." ".$lname;
						
					echo "<a href='agent_cust_receiver.php?fname=$fname & c_name=$c_name & lname=$lname & c_email=$c_email & address=$address & town=$town & county=$county & country= $country & postcode=$postcode & s_phone=$s_phone & & s_mphone=$s_mphone '><strong><li onClick='fill(\''.ucfirst($resultst->fname).'\');'>".strtoupper($fname). ' '.strtoupper($lname).'</li></a>';
