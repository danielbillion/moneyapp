...
<caption><EM><strong><u> <?php echo strtoupper($_GET['ag_name']); ?></u>: Payment Clearance </strong></EM></caption><p>
...
$admn=$ag_name;
 ...
 include "info.php";
if(($bs_mtls=='1')){
 $check3 = mysql_query("SELECT * FROM cust_transaction WHERE `amt_send` <= $bs_mtl AND comp ='$ag_name'  && status='PAID' && mc='0'")or die(mysql_error());
 $checkcla=  mysql_query("SELECT * FROM cust_transaction WHERE `amt_send` <= $bs_mtl AND comp ='$ag_name' && mc='0'")or die(mysql_error());
	
$checkcl1=  mysql_query("SELECT * FROM cust_transaction WHERE `amt_send` <= $bs_mtl AND comp ='$ag_name' && mc='0'  ")or die(mysql_error());
 }
 
 if(($bs_mtls=='0')){
 $check3 = mysql_query("SELECT * FROM cust_transaction WHERE comp ='$ag_name'   && status='PAID'  && mc='0'")or die(mysql_error());
 
 $checkcla=  mysql_query("SELECT * FROM cust_transaction WHERE comp ='$ag_name'  && mc='0'")or die(mysql_error());
	
$checkcl1=  mysql_query("SELECT * FROM cust_transaction WHERE comp ='$ag_name' && mc='0' ")or die(mysql_error());
 }



$total_t=0;$total_lc=0;$total_amtp=0;$total_lc=0;$total_ag=0;$total_comd=0;
...
// $checkp = mysql_query("SELECT * FROM cust_transaction WHERE agent_email='$age_email' && status='PAID' ")or die(mysql_error());
...
echo "</td><td align='center'>";echo $row2['man_date'];
...
if($level==1 || $level==2){
echo "</td><td align='center'>";echo "<a href='$ac?receiptno=$receiptno & admn=$admn & age_email=$age_email & r_amtsend=$r_amtsend & clpcc=clpcc'>Appv</a>";}
...
//$total_lc = $total_lc + $row2['amt_local'];
echo "</td></tr>";
...
echo "</td><td colspan='5' align='center'>";echo "";


echo "</td><td align='center'>";echo number_format($total_amtp,2) ;
echo "</td><td align='center'>";echo  number_format($total_lc,2) ;
...
echo "</table>";
	...
	$_SESSION['admn']=$admn;
	echo" <script type='text/javascript'> {window.location='admin_pay_agent.php?clpclc=clpclc' }</script> ";
