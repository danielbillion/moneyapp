...
<caption><EM>Customer Transactions Till Date P= PENDING,  S=SUSPENDED</EM></caption>
...
$start_from = ($page-1) * 20;
 ...
 include "info.php";
if(($bs_mtls=='1')){
$check = mysql_query("SELECT * FROM cust_transaction WHERE `amt_send` <= $bs_mtl AND (`status`='Pending' || `status`=' Pending' ||`status`='SUSPENDED' )  ORDER BY date DESC LIMIT $start_from, 20") or die('Query failed: ' . mysql_error());
}

if(($bs_mtls=='0')){
$check = mysql_query("SELECT * FROM cust_transaction WHERE `status`=' Pending' || `status`='Pending' || `status`='SUSPENDED'   ORDER BY date DESC LIMIT $start_from, 20") or die('Query failed: ' . mysql_error());
}
...
if(($bs_mtls=='1')){
	  $sql = "SELECT COUNT(id) FROM cust_transaction WHERE  `status`='Pending' || `status`=' Pending' || `status`='SUSPENDED' " or die('Query failed: ' . mysql_error()); 
...
for ($i=1; $i<=$total_pages; $i++) {
           echo "</td><td align='center'>"; echo "<a href='admin_custs_trans_status _report.php?page=".$i." & num=$num'>".$i."</a> ";}
