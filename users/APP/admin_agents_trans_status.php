...
<caption><EM>Agent Transactions Till Date P= PENDING,  S=SUSPENDED R= View Receipt,  N= Write a Note </EM></caption>
...
$time = strtotime($ts1);
$ts2  =date('Y-m-d', strtotime('-9 months'));
...
$start_from = ($page-1) * 25;
  ...
  if(($bs_mtls=='1')){
	   $check = mysql_query("SELECT * FROM agent_cust_transaction WHERE `amt_send` <= $bs_mtl  AND (`status`='Pending' || `status`=' Pending' || `status`='SUSPENDED')   ORDER BY dtime DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());
	  
	  }
   
   if(($bs_mtls=='0')){
	   
	  $check = mysql_query("SELECT * FROM agent_cust_transaction WHERE  `status`='Pending' ||  `status`=' Pending' || `status`='SUSPENDED'  ORDER BY dtime DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());
	   }
...
//SELECT * FROM  `agent_cust_transaction`WHERE  `amt_send` >=2000 LIMIT 0 , 30
...
if(($bs_mtls=='1')){
	  $sql = "SELECT COUNT(id) FROM agent_cust_transaction WHERE  `status`='Pending' || `status`=' Pending'  || `status`='SUSPENDED' " or die('Query failed: ' . mysql_error()); 
...
for ($i=1; $i<=$total_pages; $i++) {
           echo "</td><td align='center'>"; echo "<a href='admin_agents_trans_status.php?page=".$i." & num=$num '>".$i."</a> ";}
