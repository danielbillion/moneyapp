...
$link = include "conn.php";
 include "conn.php";
 $check = mysql_query("SELECT * FROM trans2_customer_cancel") or die('Query failed: ' . mysql_error());
 $checkac = mysql_query("SELECT * FROM trans2_customer_cancel  ORDER BY fname  ")or die(mysql_error());
 $num=0;
...
echo "</td><td align='left'>";echo "<strong><a href='#?c_email=$c_email & c_name=$c_name'> Modify Profile </a></strong>";
echo "</td><td align='left'>";echo "<strong><a href='n2uk_cust_receivers_list.php?c_email=$c_email & c_name=$c_name & town=$town & county=$county & country=$country & postcode=$postcode & address=$address & s_phone=$s_phone & s_mphone=$s_mphone  & custid=$custid'> View </a></strong>";
