...
/* Shown only for "Internal" Licenses, not Web Use */
 .SPLAddressListLicenseLine{
	...
	b_idtype.style.display = "none";
                      ...
                      <table border=1 style="background-color:#ffffFF;" align="center" cellpadding="0" cellspacing="0" >

<caption><EM>Transactions To be clear</EM></caption>
...
$start_from = "0";
 ...
 $check = mysql_query("SELECT * FROM cust_transaction") or die('Query failed: ' . mysql_error());
  ...
  include "info.php";
if(($bs_mtls=='1')){
 $check3 = mysql_query("SELECT * FROM cust_transaction WHERE `amt_send` <= $bs_mtl AND sender_email = '".$email." ' && clear='uc'")or die(mysql_error());}
 
 if(($bs_mtls=='0')){
 $check3 = mysql_query("SELECT * FROM cust_transaction WHERE sender_email = '".$email." ' && clear='uc'")or die(mysql_error());}
...
if(isset($_POST['Clear'])){
	$cs_tcl=$_POST['ck'];
	$_SESSION['cs_tcl']=$_POST['ck'];;
	echo" <script type='text/javascript'> {window.location='admin_pay_agent.php?cs_tcl=$cs_tcl' }</script> ";
