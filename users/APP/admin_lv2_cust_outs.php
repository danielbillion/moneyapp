...
<caption><EM><strong><u> <?php echo strtoupper($_GET['ag_name']); ?></u>: Cleared Customer Oustanding  CT=Clear type, CLA= cleared all, CL1=Single clear </strong></EM></caption><p>
...
$admn=$ag_name;
 ...
 include "info.php";
if(($bs_mtls=='1')){
 $check3 = mysql_query("SELECT * FROM clear_trans WHERE `amt_send` <= $bs_mtl AND cla ='$ag_name' && type='cs' && adst='' ")or die(mysql_error());

$checkcla=  mysql_query("SELECT * FROM clear_trans WHERE `amt_send` <= $bs_mtl AND cla ='$ag_name' && type='cs' && adst='' && (js='CLA' ) ")or die(mysql_error());
	
$checkcl1=  mysql_query("SELECT * FROM clear_trans WHERE `amt_send` <= $bs_mtl AND cla ='$ag_name' && type='cs' && adst='' && ( js='CL1' ) ")or die(mysql_error());}

if(($bs_mtls=='0')){
 $check3 = mysql_query("SELECT * FROM clear_trans WHERE cla ='$ag_name' && type='cs' && adst='' ")or die(mysql_error());

$checkcla=  mysql_query("SELECT * FROM clear_trans WHERE cla ='$ag_name' && type='cs' && adst='' && (js='CLA' ) ")or die(mysql_error());
	
$checkcl1=  mysql_query("SELECT * FROM clear_trans WHERE cla ='$ag_name' && type='cs' && adst='' && ( js='CL1' ) ")or die(mysql_error());}
...
echo "</td><td align='center'>";echo $row2['status'];if($level==1 || $level==2){
echo "</td><td align='center'>";echo "<a href='admin_pay_agent.php?receiptno=$receiptno & admn=$admn & age_email=$age_email & r_amtsend=$r_amtsend & adappc=adappc'>Appv</a>";}
...
echo "</table>";
	...
	$tadapc=$_POST['ck'];
	$_SESSION['tadapc']=$_POST['ck'];
	$_SESSION['admn']=$admn;
	echo" <script type='text/javascript'> {window.location='admin_pay_agent.php?tadapc=$tadapc' }</script> ";
