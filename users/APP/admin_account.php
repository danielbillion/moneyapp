...
$admn=$ag_name;
 ...
 include "info.php";
if(($bs_mtls=='1')){
 $check3 = mysql_query("SELECT * FROM agent_cust_transaction WHERE `amt_send` <= $bs_mtl AND comp ='$ag_name'    && mc='0'  && status='PAID'")or die(mysql_error());
 $checkcla=  mysql_query("SELECT * FROM agent_cust_transaction WHERE `amt_send` <= $bs_mtl AND comp ='$ag_name' && mc='0'  && status='PAID'")or die(mysql_error());
	
$checkcl1=  mysql_query("SELECT * FROM agent_cust_transaction WHERE `amt_send` <= $bs_mtl AND comp ='$ag_name' && mc='0'  && status='PAID'  ")or die(mysql_error());
 }
 
 if(($bs_mtls=='0')){
 $check3 = mysql_query("SELECT * FROM agent_cust_transaction WHERE comp ='$ag_name'   && mc='0'  && status='PAID'")or die(mysql_error());
 
 $checkcla=  mysql_query("SELECT * FROM agent_cust_transaction WHERE comp ='$ag_name'  && mc='0'  && status='PAID'")or die(mysql_error());
	
$checkcl1=  mysql_query("SELECT * FROM agent_cust_transaction WHERE comp ='$ag_name' && mc='0'  && status='PAID' ")or die(mysql_error());
 }
...
if($level==1 || $level==2){
echo "</td><td align='center'>";echo "<a href='$ac?receiptno=$receiptno & admn=$admn & age_email=$age_email & r_amtsend=$r_amtsend & clpc=clpc'>Appv</a>";}
...
echo "</table>";
	...
	$_SESSION['admn']=$admn;
	echo" <script type='text/javascript'> {window.location='admin_pay_agent.php?clpcl=clpcl' }</script> ";
