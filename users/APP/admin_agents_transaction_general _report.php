...
$start_from = ($page-1) * 25;
  ...
  if(($bs_mtls=='1')){
	   $check = mysql_query("SELECT * FROM agent_cust_transaction WHERE `amt_send` <= $bs_mtl AND STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts2' AND '$ts1' ORDER BY dtime DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());
	  
	  }
   
   if(($bs_mtls=='0')){
	   
	  $check = mysql_query("SELECT * FROM agent_cust_transaction WHERE  STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts2' AND '$ts1' ORDER BY dtime DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());
	   }
...
$sql = "SELECT COUNT(id) FROM agent_cust_transaction WHERE  STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts2' AND '$ts1' " or die('Query failed: ' . mysql_error()); 
...
for ($i=1; $i<=$total_pages; $i++) {
           echo "</td><td align='center'>"; echo "<a href='admin_agents_transaction_general _report.php?page=".$i." & num=$num '>".$i."</a> ";}
