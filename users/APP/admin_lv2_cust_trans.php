...
if(($bs_mtls=='1')){
 $check = mysql_query("SELECT * FROM cust_transaction WHERE `amt_send` <= $bs_mtl AND (STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts2' AND '$ts1' )&& (comp='$ag_name') ORDER BY date DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());}
 
 if(($bs_mtls=='0')){
 $check = mysql_query("SELECT * FROM cust_transaction WHERE  (STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts2' AND '$ts1' )&& (comp='$ag_name') ORDER BY date DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());}
 
$check3 = mysql_query("SELECT * FROM cust_transaction  ORDER BY date DESC ")or die(mysql_error());
$check1 = mysql_query("SELECT * FROM cust_transaction ")or die(mysql_error());
 //
...
$sql = "SELECT COUNT(id) FROM cust_transaction WHERE  (STR_TO_DATE(date, '%Y-%m-%d') BETWEEN '$ts2' AND '$ts1' )&& (comp='$ag_name')" or die('Query failed: ' . mysql_error()); 
...
for ($i=1; $i<=$total_pages; $i++) {
           echo "</td><td align='center'>"; echo "<a href='admin_lv2_cust_trans.php?page=".$i." & num=$num &ag_name=$ag_name '>".$i."</a> ";}
