...
//echo $email;
 ...
 $check0 = mysql_query("SELECT * FROM new_customer WHERE email ='".$email."'")or die(mysql_error());
...
$n_mnumber = mysql_escape_string($_POST['n_mnumber']);
	...
	mysql_query("UPDATE new_customer SET pnumber = '$n_pnumber', mnumber = '$n_mnumber' , email='$n_email' 
	 WHERE fname = '$fname'  && lname='$lname' ");
	
	 mysql_query("UPDATE cust_transaction SET  sender_email='$n_email' WHERE sender_email = '$email' ");
	  mysql_query("UPDATE receiver SET  sender_email='$n_email' WHERE sender_email = '$email' ");
    ...
    <th width="568" height="395" scope="row"><form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
      <table width="558" height="407" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
        ...
        <tr>
          ...
          <th scope="row"><label>
            <input name="n_pnumber"  maxlength="11" type="text"  value="<?php if (isset($pnumber)) { echo strtoupper($pnumber);} ?>" />
          </label></th>
          <th scope="row">&nbsp;</th>
