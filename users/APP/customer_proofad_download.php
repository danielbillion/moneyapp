...
$agent_em= $_GET['c_ag_email'];
 ...
 //echo $agent_em ;
 $sql = "SELECT proofad_name, proofad_type, proofad_size, proofad_content FROM new_customer WHERE email=$agent_em";
 
   $check = mysql_query("SELECT * FROM new_customer") or die('Query failed: ' . mysql_error());
   
 $check0 = mysql_query("SELECT proofad_name, proofad_type, proofad_size, proofad_content " ."FROM new_customer WHERE email ='".$agent_em."'")or die(mysql_error());
$table_array = mysql_fetch_array($check);
$result = @mysql_query($check0);


$sql = "SELECT proofad_content FROM new_customer WHERE email ='".$agent_em."'";
$sql2 = "SELECT proofad_name FROM new_customer WHERE email ='".$agent_em."'";
