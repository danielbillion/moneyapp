...
if(isset($_POST['submit'])){
  ...
  ".$arrayField['country'];
	...
	$requiredField=array('fName','lName','mNumber','dob','postcode','title');
						...
						if(($arrayField['email']!="") || ($arrayField['email']  !="") ){
							if($arrayField['email'] == $arrayField['cEmail'] ){
								...
								if($validate->isValidEmail($arrayField['email'])){
									if($senders->insertNew($arrayField)){
										
										redirect('newSender.php', 'New Sender Has Now Been Registered','success');
									}
									else{
