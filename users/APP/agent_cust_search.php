...
$email=  ($_SESSION['email']);
$custfn=mysql_escape_string($_GET['info']);
echo $custfn;

$link = include "conn.php";
 include "conn.php";  
 $check = mysql_query("SELECT * FROM agent_new_customer") or die('Query failed: ' . mysql_error());
 //$check0 = mysql_query("SELECT * FROM agent_new_customer WHERE agent_email = '$email' ORDER BY fname  ")or die(mysql_error());
 
 $check0 = mysql_query("SELECT * FROM agent_new_customer WHERE agent_email = '$email' && (fname='$custfn' || pnumber='$custfn' || lname='$custfn' || mnumber='$custfn') ORDER BY fname  ")or die(mysql_error());
...
echo "</td><td align='left'>";echo "<strong><a href='agent_cust_receiver.php?c_email=$c_email & c_name=$c_name & address=$address & s_phone=$s_phone & & s_mphone=$s_mphone''> Create Reciepient </a></strong>";
echo "</td><td align='left'>";echo "<strong><a href='agent_cust_receivers_list.php?c_email=$c_email & c_name=$c_name & town=$town & county=$county & country=$country & postcode=$postcode & address=$address & s_phone=$s_phone & s_mphone=$s_mphone'> View </a></strong>";
echo "</td><td align='left'>";echo "<strong><a href='agent_cust_edit.php?c_email=$c_email & c_name=$c_name'> Modify Profile </a></strong>";
