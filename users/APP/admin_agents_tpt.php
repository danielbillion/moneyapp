...
$start_from = ($page-1) * 25;
  ...
  if(($bs_mtls=='1')){
	   $check = mysql_query("SELECT * FROM agent_cust_transaction WHERE `amt_send` <= $bs_mtl  AND man_date='$tdate' AND agent_email='$age_email'   ORDER BY dtime DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());
	  
	  }
   
   if(($bs_mtls=='0')){
	   
	  $check = mysql_query("SELECT * FROM agent_cust_transaction WHERE  man_date='$tdate' &&  agent_email='$age_email'    ORDER BY dtime DESC LIMIT $start_from, 25") or die('Query failed: ' . mysql_error());
	   }
